...
(async () => {
  ...
  await page.evaluateOnNewDocument(() => {
    ...
    localStorage.setItem('wealthtrack-theme', JSON.stringify("light"));
  });
  await page.goto('http://localhost:5173', { waitUntil: 'networkidle0' });
  const start = await page.evaluateHandle(() => [...document.querySelectorAll('button')].find(b => b.textContent.includes('Commencer gratuit')));
  ...
  await page.screenshot({ path: '/tmp/lightmode.png' });
  console.log('done');
  await browser.close();
})();
