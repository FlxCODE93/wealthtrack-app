...
const PROFILE_CONFIG = {
  salarie_stable: {
    label: "Salarié CDI", color: "#22c79a", revenueRatio: 1.0, capacityMult: 1.0, note: null,
  },
  interimaire: {
    label: "Intérimaire / Variable", color: "#f5a623", revenueRatio: 0.85, capacityMult: 0.7,
  ...
  independant: {
    label: "Indépendant / Freelance", color: "#f5a623", revenueRatio: 0.70, capacityMult: 0.5,
    note: "Statut indépendant : les banques retiennent 70 % des revenus déclarés et exigent 2–3 ans de bilans comptables.",
  },
};

/* ------------------------------------------------------------------ */
/*  AccordionSection — section pliable avec barre colorée              */
/* ------------------------------------------------------------------ */
  ...
  return (
    <div style={{ background: T.panel, border: `1px solid ${T.border}`, borderRadius: 14, overflow: "hidden" }}>
      ...
      >
        ...
        {summaryValue && !open && (
          <span style={{ fontSize: 14, fontWeight: 800, color: summaryColor || T.text }}>{summaryValue}</span>
        )}
...
/* ------------------------------------------------------------------ */
/*  ModeCard — carte de sélection de mode                              */
/* ------------------------------------------------------------------ */
function ModeCard({ icon: Icon, label, desc, active, onClick }) {
  ...
  return (
    ...
    >
      {active && (
        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 2, background: T.blue }} />
      )}
      <div style={{
...
/* ------------------------------------------------------------------ */
/*  HeroKpis — bande KPIs horizontale                                  */
/* ------------------------------------------------------------------ */
function HeroKpis({ kpis, badge }) {
  const T = useT();
  return (
    <div style={{
      ...
      {kpis.map((kpi, i) => (
        <div key={i} style={{
          flex: 1, minWidth: 120, padding: "0 24px",
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-3">
        ...
        <AccordionSection accentColor={T.cyan} title="Capacité d'emprunt" desc="Règle des 35 % HCSF · profil détecté · crédits existants" summaryValue={eur(loan20)} summaryColor={T.cyan}>
          <div style={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: 8, borderRadius: 12, padding: "10px 16px", marginBottom: 16, background: profileType === "salarie_stable" ? "rgba(34,199,154,0.06)" : "rgba(245,166,35,0.06)", border: `1px solid ${profileType === "salarie_stable" ? "rgba(34,199,154,0.3)" : "rgba(245,166,35,0.3)"}` }}>
          ...
          <div style={{ borderRadius: 12, padding: 16, background: canAfford ? "rgba(34,199,154,0.06)" : "rgba(255,90,95,0.06)", border: `1px solid ${canAfford ? T.green + "44" : T.red + "44"}` }}>
            ...
            {!canAfford && (
              <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 4, fontSize: 12, color: T.muted }}>
                <div>• Attendre {Math.ceil(affordGap / Math.max(1, simParams?.monthly || 1000))} mois d'épargne</div>
        ...
        {/* 4 — Projection */}
        <AccordionSection accentColor={T.green} title={`Projection sur ${duration} ans`} desc="Valeur du bien · patrimoine net · comparaison achat vs location" summaryValue={eur(finalEquity)} summaryColor={T.green}>
          ...
          {showRentVsBuy && (
            <>
              ...
              <div style={{ borderRadius: 10, padding: "10px 14px", marginBottom: 16, fontSize: 12, display: "flex", flexWrap: "wrap", gap: "4px 8px", alignItems: "center", background: `rgba(8,145,178,0.05)`, border: `1px solid ${T.cyan}33`, color: T.muted }}>
                {investMonthly > 0 ? (
                  <>
...
function LocatifSection({ T, revenueForBank, bCfg, autoCredits }) {
  ...
  const loan25sans = Math.round(loanFromPayment(mensualiteMax, 0.037, 25));
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-3">
        ...
        <AccordionSection accentColor={T.cyan} title="Capacité d'emprunt investisseur" desc="Les banques intègrent 70 % du loyer prévisionnel dans le calcul du taux d'endettement" summaryValue={eur(locBudgetSearch)} summaryColor={T.cyan}>
          <div style={{ borderRadius: 10, padding: "12px 16px", background: T.panel, border: `1px solid ${T.border}` }}>
            ...
            <div style={{ fontSize: 12, color: T.muted }}>
              ...
              (au lieu de {eur(loan25sans)} sans loyer pris en compte).
            </div>
          </div>
        </AccordionSection>
        ...
        <AccordionSection accentColor={T.blue} title="Paramètres du bien locatif" desc="Prix, apport, taux, loyer, charges, vacance" summaryValue={eur(locPrice)} summaryColor={T.blue} defaultOpen={true}>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <Field label={`Apport, notaire inclus (${locApportPct} % = ${eur(locTotalApport)})`}>
              <input type="range" min={5} max={50} step={1} value={locApportPct} onChange={(e) => setLocApportPct(+e.target.value)} className="w-full" style={{ accentColor: T.blue }} />
            </Field>
            <Field label={`Vacance locative (${locVacance} mois/an)`}>
              <input type="range" min={0} max={3} step={1} value={locVacance} onChange={(e) => setLocVacance(+e.target.value)} className="w-full" style={{ accentColor: T.amber }} />
            </Field>
            <Field label="Taux crédit (% / an)"><NumInput value={locRate} step={0.1} style={inputStyle} onChange={(n) => setLocRate(n)} /></Field>
            <Field label="Durée du crédit">
              <select value={locDuration} onChange={(e) => setLocDuration(+e.target.value)} style={inputStyle}>
                {[10, 15, 20, 25, 30].map((d) => <option key={d} value={d}>{d} ans</option>)}
              </select>
            </Field>
            <Field label="Loyer mensuel hors charges (€)"><NumInput value={locLoyer} style={inputStyle} onChange={(n) => setLocLoyer(n)} /></Field>
            <Field label="Charges copropriété non récupérables (€/mois)"><NumInput value={locCharges} style={inputStyle} onChange={(n) => setLocCharges(n)} /></Field>
            <Field label="Taxe foncière (€/an)"><NumInput value={locTaxeFonciere} style={inputStyle} onChange={(n) => setLocTaxeFonciere(n)} /></Field>
            <Field label={<>Assurance PNO (€/an)<InfoTooltip text="Assurance Propriétaire Non Occupant." /></>}><NumInput value={locAssurancePNO} style={inputStyle} onChange={(n) => setLocAssurancePNO(n)} /></Field>
            <Field label={<>Assurance emprunteur (% / an du capital)<InfoTooltip text="Coût indicatif : 0,10 % à 0,60 % selon l'âge et le profil." /></>}><NumInput value={locAssuranceEmprunteurPct} step={0.05} style={inputStyle} onChange={(n) => setLocAssuranceEmprunteurPct(n)} /></Field>
            <Field label="Appréciation annuelle du bien (%)"><NumInput value={locAppreciation} step={0.5} style={inputStyle} onChange={(n) => setLocAppreciation(n)} /></Field>
          </div>
        </AccordionSection>
        ...
        <AccordionSection accentColor={T.cyan} title="Cash-flow mensuel" desc="Avant impôt · détail ligne à ligne" summaryValue={`${cashflowMensuel >= 0 ? "+" : "−"}${eur(Math.abs(Math.round(cashflowMensuel)))}/mois`} summaryColor={cashflowMensuel >= 0 ? T.green : T.red}>
          ...
          <div style={{ marginTop: 16, paddingTop: 12, borderTop: `1px solid ${T.border}`, fontSize: 11, color: T.muted, lineHeight: 1.6 }}>
            ...
            Cash-flow <b>avant impôt sur les revenus fonciers</b>. Consultez <b>Fiscalité → Revenus locatifs</b> pour le détail.
          </div>
        </AccordionSection>

        {/* Projection locative — NOUVEAU */}
        <AccordionSection accentColor={T.green} title={`Projection sur ${locDuration} ans`} desc="Valeur du bien · capital net après remboursement du crédit" summaryValue={eur(locFinalEquity)} summaryColor={T.green}>
      ...
      <div className="flex flex-col gap-3">
        ...
        <AccordionSection accentColor={T.blue} title="Paramètres de la mise en location" desc="Loyer, crédit, charges, vacance, code postal" summaryValue={`${eur(melLoyerBrut)}/mois`} summaryColor={T.blue} defaultOpen={true}>
          <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", fontSize: 13, marginBottom: 16, userSelect: "none", borderRadius: 10, padding: "10px 14px", background: melMeuble ? "rgba(34,199,154,0.06)" : "rgba(255,255,255,0.02)", border: `1px solid ${melMeuble ? T.green + "44" : T.border}` }}>
            ...
            <InfoTooltip text="Cochez si le bien est loué avec mobilier. Définit le régime fiscal : micro-BIC (50 %) pour le meublé, micro-foncier (30 %) pour la location nue." align="left" />
          </label>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <Field label="Surface du bien (m²)"><NumInput value={melSurface} style={inputStyle} onChange={(n) => setMelSurface(n)} /></Field>
            <Field label="Loyer mensuel brut estimé (€)"><NumInput value={melLoyerBrut} style={inputStyle} onChange={(n) => setMelLoyerBrut(n)} /></Field>
            <Field label={<>Mensualité de crédit, assurances comprises (€)<InfoTooltip text="0 si le bien est entièrement remboursé." align="left" /></>}><NumInput value={melMensualite} style={inputStyle} onChange={(n) => setMelMensualite(n)} /></Field>
            <Field label={`Vacance locative (${melVacance} mois/an)`}>
              <input type="range" min={0} max={3} step={1} value={melVacance} onChange={(e) => setMelVacance(+e.target.value)} className="w-full" style={{ accentColor: T.amber }} />
            </Field>
            <Field label="Charges copropriété non récupérables (€/mois)"><NumInput value={melChargesCopro} style={inputStyle} onChange={(n) => setMelChargesCopro(n)} /></Field>
            <Field label="Taxe foncière (€/an)"><NumInput value={melTaxeFonciere} style={inputStyle} onChange={(n) => setMelTaxeFonciere(n)} /></Field>
            <Field label={<>Assurance PNO (€/an)<InfoTooltip text="Assurance Propriétaire Non Occupant." /></>}><NumInput value={melPNO} style={inputStyle} onChange={(n) => setMelPNO(n)} /></Field>
            <Field label={<>GLI — Garantie Loyers Impayés (% du loyer)<InfoTooltip text="Coût indicatif : 2 % à 3,5 % du loyer." /></>}><NumInput value={melGLIPct} step={0.1} style={inputStyle} onChange={(n) => setMelGLIPct(n)} /></Field>
            <Field label={<>Frais de gestion locative (% du loyer)<InfoTooltip text="5 % à 8 % si agence, 0 si gestion directe." /></>}><NumInput value={melGestionPct} step={0.5} style={inputStyle} onChange={(n) => setMelGestionPct(n)} /></Field>
            <Field label={<>Provision entretien / travaux (% du loyer)<InfoTooltip text="Recommandé : 5 % à 10 % du loyer." /></>}><NumInput value={melEntretienPct} step={0.5} style={inputStyle} onChange={(n) => setMelEntretienPct(n)} /></Field>
          </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  ...
                  {melTaxeFonciereEstimee != null && (
                    <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 8, fontSize: 12, color: T.muted }}>
                      ...
                      <button onClick={() => setMelTaxeFonciere(melTaxeFonciereEstimee)} style={{ padding: "3px 10px", borderRadius: 999, border: `1px solid ${T.blue}55`, background: "rgba(91,141,239,0.12)", color: T.blue, fontWeight: 700, fontSize: 12, cursor: "pointer" }}>Utiliser</button>
                    </div>
                  )}
                  <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 8, fontSize: 12, color: T.muted }}>
                    <span>Assurance PNO estimée : <b style={{ color: T.text }}>{eur(melPNOEstimee)}/an</b></span>
                    <button onClick={() => setMelPNO(melPNOEstimee)} style={{ padding: "3px 10px", borderRadius: 999, border: `1px solid ${T.blue}55`, background: "rgba(91,141,239,0.12)", color: T.blue, fontWeight: 700, fontSize: 12, cursor: "pointer" }}>Utiliser</button>
                  </div>
        ...
        <AccordionSection accentColor={T.violet} title="Imposition des revenus locatifs" desc="Régime fiscal · TMI · base imposable · déficit foncier" summaryValue={eur(Math.round(melImpotAnnuel))} summaryColor={T.red}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            <Field label={<>Régime fiscal<InfoTooltip text="Micro-foncier (nu, 30 %) ou micro-BIC (meublé, 50 %). Le régime réel déduit les charges réelles." align="left" /></>}>
              <select value={melRegimeFiscal} style={inputStyle} onChange={(e) => setMelRegimeFiscal(e.target.value)}>
                {melMeuble ? (
                  <><option value="microBic">Micro-BIC — meublé (abattement 50 %)</option><option value="reel">Régime réel LMNP (déduction des charges)</option></>
                ) : (
                  <><option value="micro">Micro-foncier — nu (abattement 30 %)</option><option value="reel">Régime réel (déduction des charges)</option></>
                )}
              </select>
            </Field>
        ...
        <AccordionSection accentColor={T.cyan} title="Cash-flow mensuel de la location" desc="Avant et après impôt · détail ligne à ligne" summaryValue={`${melCashflowApresImpot >= 0 ? "+" : "−"}${eur(Math.abs(Math.round(melCashflowApresImpot)))}/mois`} summaryColor={melCashflowApresImpot >= 0 ? T.green : T.red}>
          <div style={{ display: "flex", flexDirection: "column", gap: 8, fontSize: 13 }}>
            {[
        ...
        <AccordionSection accentColor={T.green} title="Location nue vs meublée (LMNP)" desc="Comparatif cash-flow net après impôt selon le statut" summaryValue={melDeltaMeuble >= 0 ? `meublé +${eur(Math.round(melDeltaMeuble))}/mois` : `nu +${eur(Math.round(-melDeltaMeuble))}/mois`} summaryColor={melDeltaMeuble >= 0 ? T.green : T.muted}>
          <Field label={<>Majoration loyer en meublé ({melLoyerMeubleMajorationPct} % = {eur(melLoyerMeuble)}/mois)<InfoTooltip text="Un bien meublé se loue généralement 10 à 20 % plus cher qu'un bien nu." align="left" /></>}>
            <input type="range" min={0} max={30} step={1} value={melLoyerMeubleMajorationPct} onChange={(e) => setMelLoyerMeubleMajorationPct(+e.target.value)} className="w-full" style={{ accentColor: T.green }} />
          </Field>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
            {[
              { label: "Location nue", active: !melMeuble, scenario: melScenarioNu, regime: melRegimeNuComparaison === "micro" ? "Micro-foncier — abattement 30 %" : "Régime réel — déduction charges", bg: "rgba(255,255,255,0.02)", bdr: T.border },
              { label: "Location meublée", active: melMeuble, scenario: melScenarioMeuble, regime: "Micro-BIC — abattement 50 %", bg: "rgba(34,199,154,0.04)", bdr: `${T.green}33` },
            ].map(({ label, active, scenario, regime, bg, bdr }) => (
              <div key={label} style={{ borderRadius: 12, padding: 16, background: bg, border: `1px solid ${bdr}` }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: T.text }}>{label}</div>
                  {active && <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 999, background: `${T.green}22`, color: T.green }}>Configuration actuelle</span>}
                </div>
...
/* ------------------------------------------------------------------ */
/*  Immobilier — composant principal                                   */
/* ------------------------------------------------------------------ */
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
        <ModeCard icon={Home} label="Résidence principale" desc="Achat, mensualité, achat vs location" active={mode === "residence"} onClick={() => setMode("residence")} />
        <ModeCard icon={Building2} label="Investissement locatif" desc="Rendement, cash-flow, crédit investisseur" active={mode === "locatif"} onClick={() => setMode("locatif")} />
        <ModeCard icon={Key} label="Mettre en location" desc="Bien existant, LMNP, imposition nette" active={mode === "location"} onClick={() => setMode("location")} />
      </div>

      {mode === "residence" && (
        <ResidenceSection T={T} netWorth={netWorth} revenueForBank={revenueForBank} bCfg={bCfg} profileType={profileType} autoCredits={autoCredits} simParams={simParams} />
      )}
      {mode === "locatif" && (
        <LocatifSection T={T} revenueForBank={revenueForBank} bCfg={bCfg} autoCredits={autoCredits} />
      )}
      {mode === "location" && (
        <LocationSection T={T} />
      )}
