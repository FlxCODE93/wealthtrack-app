...
export default function Landing({ onStart }) {
  ...
  return (
    <div style={{ color: T.text, fontFamily: "'Geist Sans', 'Inter', -apple-system, 'Segoe UI', sans-serif", minHeight: "100vh", position: "relative", overflow: "hidden" }}>
      ...
      <section className="px-6 md:px-16 mb-20 max-w-5xl mx-auto">
        <div className="text-center mb-10">
          <h2 className="text-3xl md:text-4xl font-black mb-3" style={{ color: T.text }}>
            Comprendre les frais & dispositifs
          </h2>
          <p className="text-base" style={{ color: T.muted }}>
            1% de frais en plus sur 30 ans peut effacer 26% de votre capital. Voici ce qu'il faut savoir.
          </p>
        </div>
          ...
          {[
            {
              ...
              color: "#ef4444",
              icon: "✗",
              desc: "Prélevés une seule fois à la souscription (ex: 3% sur 10 000€ = 300€ perdus dès le départ). Négociables ou nuls en ligne.",
              tip: "Visez 0% — ils n'existent plus chez les courtiers en ligne.",
            },
            {
              title: "Frais de gestion annuels",
              color: "#f59e0b",
              icon: "%",
              desc: "Prélevés chaque année sur votre encours. L'effet de capitalisation les rend dévastateurs : 1,5%/an sur 20 ans = ~25% du capital final sacrifié.",
              tip: "ETF PEA : ~0,25%/an. Fonds actifs : 1,5–2,5%/an.",
            },
            {
              title: "Commission de surperformance",
              color: "#8b5cf6",
              icon: "⚠",
              desc: "Certains fonds prélèvent 10–20% des gains qui dépassent leur indice. Applicable même si votre capital a baissé en absolu.",
              tip: "Préférer les fonds sans commission de performance.",
            },
          ].map((f) => (
            <div key={f.title} className="rounded-2xl p-5" style={{ background: T.veil1, border: `1px solid ${f.color}33` }}>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="font-bold text-sm" style={{ color: T.text }}>{f.title}</span>
              </div>
              <p className="text-xs leading-relaxed mb-3" style={{ color: T.muted }}>{f.desc}</p>
              <div className="text-xs px-3 py-2 rounded-lg font-semibold" style={{ background: f.color + "10", color: f.color }}>
                {f.tip}
              </div>
            </div>
          ))}
        ...
        {/* Dispositifs comparatifs */}
        <div className="rounded-2xl overflow-hidden mb-8" style={{ border: `1px solid ${T.border}` }}>
          <div className="px-5 py-4" style={{ background: T.veil1, borderBottom: `1px solid ${T.border}` }}>
            <h3 className="font-bold text-base" style={{ color: T.text }}>Quel dispositif choisir ?</h3>
          </div>
          <div className="divide-y" style={{ borderColor: T.border }}>
            {[
              { name: "ETF PEA", frais: "~0,25%/an", rendement: "~10,5%/an historique", verdict: "Meilleur rapport coût/performance", color: "#22c55e" },
              { name: "Assurance-Vie (UC)", frais: "~1,4%/an", rendement: "Selon supports choisis", verdict: "Avantage successoral et fiscal", color: "#3b82f6" },
              { name: "PER", frais: "~1,5%/an", rendement: "Selon supports", verdict: "Déduction fiscale à l'entrée (retraite)", color: "#8b5cf6" },
              { name: "AV fonds euros", frais: "~0,8%/an", rendement: "~2,5%/an brut (2024)", verdict: "Capital garanti — épargne de précaution", color: "#f59e0b" },
              { name: "OPCVM actifs", frais: "2–4% entrée + ~2%/an", rendement: "Souvent < indice (SPIVA)", verdict: "80% sous-performent l'indice sur 10 ans", color: "#ef4444" },
              { name: "SCPI", frais: "8–12% entrée + ~1%/an", rendement: "~4–5% distribution brute", verdict: "Pierre-papier — horizon 10+ ans", color: "#f97316" },
            ].map((d) => (
              <div key={d.name} className="flex items-center gap-4 px-5 py-3 flex-wrap" style={{ background: "transparent" }}>
                <div style={{ width: 8, height: 8, borderRadius: 4, background: d.color, flexShrink: 0 }} />
                <div className="font-semibold text-sm w-36 shrink-0" style={{ color: T.text }}>{d.name}</div>
                <div className="text-xs" style={{ color: "#ef4444", minWidth: 140 }}>{d.frais}</div>
                <div className="text-xs hidden sm:block" style={{ color: T.muted, flex: 1 }}>{d.rendement}</div>
                <div className="text-xs font-medium" style={{ color: T.muted }}>{d.verdict}</div>
              </div>
            ))}
          </div>
        </div>

        {/* Gestion libre vs pilotée + UC vs fonds € */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="rounded-2xl p-5" style={{ background: T.veil1, border: `1px solid ${T.border}` }}>
            <h3 className="font-bold text-sm mb-3" style={{ color: T.text }}>Gestion libre vs gestion pilotée</h3>
            <div className="space-y-2 text-xs" style={{ color: T.muted }}>
              <p><strong style={{ color: "#22c55e" }}>Gestion libre :</strong> vous choisissez vous-même les supports (ETF, fonds €, UC). Coût minimal, contrôle total. Nécessite de vous y intéresser un peu.</p>
              <p><strong style={{ color: "#3b82f6" }}>Gestion pilotée :</strong> un algorithme répartit selon votre profil. Frais supplémentaires ~0,3–0,9%/an. Pratique, mais coûteux sur le long terme.</p>
              <div className="mt-3 px-3 py-2 rounded-lg" style={{ background: "rgba(34,197,94,0.08)", color: "#22c55e" }}>
                Règle : si vous pouvez nommer ce que vous investissez, choisissez la gestion libre.
              </div>
            </div>
          </div>
          <div className="rounded-2xl p-5" style={{ background: T.veil1, border: `1px solid ${T.border}` }}>
            <h3 className="font-bold text-sm mb-3" style={{ color: T.text }}>Unités de Compte vs fonds euros</h3>
            <div className="space-y-2 text-xs" style={{ color: T.muted }}>
              <p><strong style={{ color: "#f59e0b" }}>Fonds euros :</strong> capital garanti, rendement ~2–3%. Idéal pour la partie sécurisée de votre AV ou PER. Risque quasi-nul.</p>
              <p><strong style={{ color: "#3b82f6" }}>Unités de Compte (UC) :</strong> supports non garantis (actions, ETF, SCPI…). Potentiel de rendement supérieur, risque de perte en capital.</p>
              <div className="mt-3 px-3 py-2 rounded-lg" style={{ background: "rgba(245,158,11,0.08)", color: "#f59e0b" }}>
                Règle : jeune = UC dominants ; proche retraite = glissement progressif vers fonds €.
              </div>
            </div>
          </div>
        </div>
